...
def get_datas(prox: bool, anoin, mesin):
    ...
    if(prox):  # prox é a variável booleana que define se você quer o próximo mês ou o anterior
        mes_anterior = (data + dt.timedelta(days=31)
                        ).month  # Pega o mês seguinte
        # Pega o ano em que esse próximo mês está
        ano_anterior = (data + dt.timedelta(days=31)).year
        meses = [mes_atual, mes_anterior]
        anos = [ano_atual, ano_anterior]
    else:
        mes_anterior = (data - dt.timedelta(days=31)
                        ).month  # Pega o mês anterior
        # Pega o ano em que esse mês anterior está
        ano_anterior = (data - dt.timedelta(days=31)).year
        meses = [mes_anterior, mes_atual]
        anos = [ano_anterior, ano_atual]
    # Em ambos os casos, os vetores são construídos da menor para a maior data
    return meses, anos


# Função que nomeia os meses escolhidos pelas funções de data
# Para funcionar propriamente, tudo o que depende DIRETAMENTE da data
def get_nome(prox, anoin, mesin):
    # precisa de um parâmetro booleano
    # Seleciona as datas apropriadas em relação ao prox
    meses, anos = get_datas(prox, anoin, mesin)
    nomes = []
    for mes in meses:  # Só um "switch-case" para escrever o nome apropriado de cada mês
        if mes == 1:
            nomes.append('Janeiro')
        elif mes == 2:
            nomes.append('Fevereiro')
        elif mes == 3:
            nomes.append('Março')
        elif mes == 4:
            nomes.append('Abril')
            ...
            nomes.append('Setembro')
        elif mes == 10:
            nomes.append('Outubro')
        elif mes == 11:
            nomes.append('Novembro')
        elif mes == 12:
            nomes.append('Dezembro')
    return nomes, meses, anos


# Função gera os nomes dos arquivos dos meses apropriados
def gera_nome(prox, anoin, mesin):
    # Pega os nomes apropriados de acordo com prox
    nomes, meses, anos = get_nome(prox, anoin, mesin)
    for i, nome in enumerate(nomes):
        # Formata o local para a busca de arquivos de PMO
...
def compara_meses(anoin, mesin):
    ...
    # Pega a data atual e a próxima
    meses, anos = get_datas(True, anoin, mesin)
    datas = []
    ...
    tab_mes0.query(
        'TYPE == "MEDIUM" and (DATE == @datas[0] or DATE == @datas[1])', inplace=True)
    tab_mes1.query(
        'TYPE == "MEDIUM" and (DATE == @datas[0] or DATE == @datas[1])', inplace=True)
    ...
    # csem recebe um vetor com as diferenças de previsão do PMO e do DECOMP
    ...
    SE = tab_mes0.query('SOURCE == "SUDESTE"')[
        'LOAD'] - tab_mes1.query('SOURCE == "SUDESTE"')['LOAD']
    S = tab_mes0.query('SOURCE == "SUL"')[
        'LOAD'] - tab_mes1.query('SOURCE == "SUL"')['LOAD']
    NE = tab_mes0.query('SOURCE == "NORDESTE"')[
        'LOAD'] - tab_mes1.query('SOURCE == "NORDESTE"')['LOAD']
    N = tab_mes0.query('SOURCE == "NORTE"')[
        'LOAD'] - tab_mes1.query('SOURCE == "NORTE"')['LOAD']
...
def compara_semanal(tab1, mes, ano):
    # Cria o diretório para o arquivo exportado (que é da revisão mais recente)
    ...
    # índice da tabela onde se encontra a previsão do próximo mês
    ind_prev = tab.index.size-1
    # Consegue o valor da diferença da previsão pra SE
    se = tab.iloc[ind_prev, 0] - tab1.loc['SUDESTE', 'LOAD']
    # Consegue o valor da diferença da previsão pra S
    s = tab.iloc[ind_prev, 1] - tab1.loc['SUL', 'LOAD']
    # Consegue o valor da diferença da previsão pra NE
    ne = tab.iloc[ind_prev, 2] - tab1.loc['NORDESTE', 'LOAD']
    # Consegue o valor da diferença da previsão pra N
    n = tab.iloc[ind_prev, 3] - tab1.loc['NORTE', 'LOAD']
...
def monta_tabela(anoin, mesin):
    ...
    ne.name = 'NE'
    n.name = 'N'
    # Cria a tabela a partir das séries
    tab = pd.concat([se, s, ne, n], axis=1)
    tab.sort_index(inplace=True)  # Ordena as datas
    # Pega nomes dos meses, os números e os anos apropriados para o mês
    nomes, meses, anos = get_nome(True, anoin, mesin)
    for i, nome in enumerate(nomes):
        # Reescreve os nomes com os anos apropriados
    ...
    # Adiciona a série criada e propriamente nomeada na tabela das diferenças do PMO
    tab = tab.append(comp_sem)
    # Esse nome é o nome do mês atual, sendo retornado para uso na exportação do arquivo
    return tab, nomes[0]


def exporta_tab(anoin, mesin):
    tab, nome = monta_tabela(anoin, mesin)
    # Exportação com o nome retornado na função anterior
