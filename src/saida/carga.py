...
def importa_carga(rv:int):
    local = Path('entradas/carga/carga_RV'+str(rv)) #Escolhe o arquivo de carga apropriado
...
def limpa_carga(carga_bruto):
    carga_por_estag = carga_bruto.split("&") # Divide o arquivo onde tem uma quebra de linha (denotada por &)
    for i in range(5):
        carga_por_estag.pop(0) #Retira o primeiro item da divisão por não ter nenhum valor significativo
...
def separa_estags(carga_por_estag):
    ...
    for item in carga_por_estag:
        carga.append(item.split('DP')) #Divide o arquivo por cada indicação de submercado 
    ...
    for item in carga:
        ...
        for i, texto in enumerate(item):
            item[i] = texto.split() #Divide cada linha de valores de submercado em todo whitespace
    return carga
...
def get_semanas(estags, mes, ano):
    ...
    data = str(ano)+'-'+str(mes)+'-'+'01' #cria uma data para ser usada de referência para o começo do mês operativo
    data_rv = dt.datetime.strptime(data, '%Y-%m-%d') #Converte a string em um objeto datetime
    no_semana = data_rv.isoweekday() % 7 #Pega o dia da semana e o transofrma em inteiro pela operação mod
    inicio = data_rv - dt.timedelta(days = no_semana + 1) # Define o início da semana operativa pegando o sábado que aconteceu antes do primeiro dia do mês
    for i in estags:
        datas.append(inicio + dt.timedelta(weeks = int(i))) #Incrementa a semana de acordo com o estágio que se refere
    return datas
...
def organiza_tabela(carga, rv, mes, ano):
    estags = np.arange(rv,rv+len(carga)) #Cria um vetor com os dias operativos restantes no mês de acordo com a revisão
    ind = get_semanas(estags, mes, ano) #Os índices são os primeiros dias de operação do mês
    col = ["SE","S","NE","N"] #As colunas são os submercados
    carga_decomp = pd.DataFrame(0, index = ind, columns = col) #O df é criado com essas especificações e preenchido com 0
    ...
    for i in range(len(carga)): #Laço de 0 ao número de estágios
        for j in range(4): #Laço de 0 a 3: um número para cada submercado
            for k in range(1,4): #Laço de 1 a 3 para pegar os elementos dos vetores de carga que possuem valores
                cargahora += float(carga[i][j][2*k+1]) * float(carga[i][j][2+2*k]) #Os laços somam carga*hora a cada iteração por uma linha
                horas += float(carga[i][j][2+2*k]) #As horas são somadas nessa variável ao longo da linha
            carga_decomp.iloc[i][j] = cargahora/horas #A média ponderada é calculada e armazenada no estágio e submercado apropriado na tabela
