...
def posto_127():
    ...
    vazao_posto_127 = vazao_posto(
        129) - posto_298() - vazao_posto(203) + posto_304()
    ...
    for i in range(30):
        if(p288.iloc[i] <= vazao_base):
            ...
            if(p288.iloc[i] <= vazao_base):
                ...
                if(p288.iloc[i] <= vazao_base + 13900):
                    print(vazao_posto_292)
                    vazao_posto_292.iloc[i] = p288.iloc[i] - vazao_base
                else:
                    vazao_posto_292.iloc[i] = 13900
...
def posto_298():
    ...
    for i in range(30):
        if(p125.iloc[i] <= 190):
            vazao_posto_298.iloc[i] = (p125.iloc[i] * 119) / 190
        elif(p125.iloc[i] <= 209):
            vazao_posto_298.iloc[i] = 119
        elif(p125.iloc[i] <= 250):
            vazao_posto_298.iloc[i] = p125.iloc[i] - 90
        elif(p125.iloc[i] > 250):
            vazao_posto_298.iloc[i] = 160
...
def posto_303():
    ...
    p316 = posto_316()
    for i in range(30):
        vazao_posto_303.iloc[i] = p132.iloc[i] + \
            min(p316.iloc[i] - p131.iloc[i], 51)
...
def posto_315():
    ...
    vazao_posto_315 = vazao_posto(
        203) - vazao_posto(201) + posto_317() + posto_298()
